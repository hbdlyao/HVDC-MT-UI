...
{
    public partial class CxbUIDevParam : DpcpBaseForm
    {
        // 监视设备map
        ...
        // 阻波器map
        private IDictionary<string, CxbDevTrap> trapMap = new Dictionary<string, CxbDevTrap>();

        public CxbUIDevParam()
        {
            InitializeComponent();
        ...
        private void dgvDkq_RowPostPaint(object sender, DataGridViewRowPostPaintEventArgs e)
        {
            ...
            dataGridViewStyle(dgvTrap, sender, e);
        }
        ...
        private void monitorApply()
        {
            ...
            for (int i = 0; i < dgvMonitor.Rows.Count; i++)
            {
                ...
                object monitorNodeFrist = dgvTrap.Rows[i].Cells["monitorNodeFrist"].Value;
                if (monitorNodeFrist == null || StringUtil.IsEmpty(monitorNodeFrist.ToString()))
                {
                    MessageBox.Show("[阻波器]第[" + (i + 1) + "]行第[3]列不能为空");
                    return;
                }
                vo.NodeNames[1] = monitorNodeFrist.ToString();

                object monitorNodeLast = dgvTrap.Rows[i].Cells["monitorNodeLast"].Value;
                if (monitorNodeLast == null || StringUtil.IsEmpty(monitorNodeLast.ToString()))
                {
                    MessageBox.Show("[阻波器]第[" + (i + 1) + "]行第[4]列不能为空");
                    return;
                }
                vo.NodeNames[0] = trapNodeLast.ToString();

                object posOrNeg = dgvMonitor.Rows[i].Cells["monitorPosOrNeg"].Value;
                if (!validatePosONeg(posOrNeg))
                {
                    MessageBox.Show("[接地极监视装置电阻]第[" + (i + 1) + "]行第[5]列必须是0或1的整数");
                    return;
                }
                vo.PosOrNeg = int.Parse(posOrNeg.ToString());
                ...
                if (zr == null)
                {
                    MessageBox.Show("请输入[接地极监视装置电阻]第[" + (i + 1) + "]行第[6]列");
                    return;
                }
                vo.Zr = double.Parse(zr.ToString());
                ...
                if (zl == null)
                {
                    MessageBox.Show("请输入[接地极监视装置电感]第[" + (i + 1) + "]行第[7]列");
                    return;
                }
                ...
                if (zc == null)
                {
                    MessageBox.Show("请输入[接地极监视装置电容]第[" + (i + 1) + "]行第[8]列");
                    return;
                }
                vo.Z_C = double.Parse(zc.ToString());
            ...
            CxbDevTrap updateMonitor = null; 
            foreach (CxbDevTrap monitor in monitorList)
            {
        ...
        /// 监视装置应用
        /// </summary>
        private void trapApply()
        {
            CDevTBL tbl = CxbVars.pxbHvdcGrid.DevTBLfromStation(getSelectedStationName(), CxbDef.xb_Trap);
            List<CDevBase> trapList = tbl.Children();
            List<CxbDevBranch> branchList = new List<CxbDevBranch>();

            string parentDevice = null; // 列表中监视设备名称
            for (int i = 0; i < dgvTrap.Rows.Count; i++)
            {
                parentDevice = dgvTrap.Rows[i].Cells["trapParentDevice"].Value.ToString();
                CxbDevBranch vo = CxbDevTBL.xbNewDevice(CxbDef.xb_Branch) as CxbDevBranch;
                vo.StationNames[0] = dgvTrap.Rows[i].Cells["trapStation"].Value.ToString();
                vo.DeviceName = dgvTrap.Rows[i].Cells["trapDevice"].Value.ToString();

                object trapNodeFrist = dgvTrap.Rows[i].Cells["trapNodeFrist"].Value;
                if (trapNodeFrist == null || StringUtil.IsEmpty(trapNodeFrist.ToString()))
                {
                    MessageBox.Show("[阻波器]第[" + (i + 1) + "]行第[3]列不能为空");
                    return;
                }
                vo.NodeNames[1] = trapNodeFrist.ToString();

                object trapNodeLast = dgvTrap.Rows[i].Cells["trapNodeLast"].Value;
                if (trapNodeLast == null || StringUtil.IsEmpty(trapNodeLast.ToString()))
                {
                    MessageBox.Show("[阻波器]第[" + (i + 1) + "]行第[4]列不能为空");
                    return;
                }
                ...
                object posOrNeg = dgvTrap.Rows[i].Cells["trapPosOrNeg"].Value;
                if (!validatePosONeg(posOrNeg))
                {
                    MessageBox.Show("[阻波器]第[" + (i + 1) + "]行第[5]列必须是0或1的整数");
                    return;
                }
                vo.PosOrNeg = int.Parse(posOrNeg.ToString());

                object zr = dgvTrap.Rows[i].Cells["trapZr"].Value;
                if (zr == null)
                {
                    MessageBox.Show("请输入[阻波器]第[" + (i + 1) + "]行第[6]列");
                    return;
                }
                vo.Zr = double.Parse(zr.ToString());

                object zl = dgvTrap.Rows[i].Cells["trapZl"].Value;
                if (zl == null)
                {
                    MessageBox.Show("请输入[阻波器]第[" + (i + 1) + "]行第[7]列");
                    return;
                }
                vo.Z_L = double.Parse(zl.ToString());

                object zc = dgvTrap.Rows[i].Cells["trapZc"].Value;
                if (zc == null)
                {
                    MessageBox.Show("请输入[阻波器]第[" + (i + 1) + "]行第[8]列");
                    return;
                }
                vo.Z_C = double.Parse(zc.ToString());
            ...
            // 找到下拉框选中的monitor，更新它的branch集合
            CxbDevTrap updateTrap = null;
            foreach (CxbDevTrap trap in trapList)
            {
                if (trap.DeviceName.Equals(parentDevice))
                {
                    updateTrap = trap;
                }
            }
            if (updateTrap != null)
            {
                updateTrap.Children().Clear();
                trapMap[parentDevice].Children().Clear();
                foreach (CxbDevBranch branch in branchList)
                {
                    updateTrap.Children().Add(branch);
                    trapMap[parentDevice].Children().Add(branch);
                }
            }
        }
        ...
        private void trapOnLoad()
        {
            ...
            foreach (CDevBase vo in list)
            {
                lstTrap.Items.Add(vo.DeviceName);
                trapMap.Add(vo.DeviceName, vo as CxbDevTrap);
            }
            if (lstTrap.Items.Count > 0)
            {
                lstTrap.SelectedIndex = 0;
            }
        }
        ...
        {
            NewDcLineUIcs vLine = new NewDcLineUIcs("监控装置");
            vLine.ShowDialog();
            if (vLine.Result == 1)
            {
                // 新增成功
        ...
        {
            NewDcLineUIcs vLine = new NewDcLineUIcs("阻波器");
            vLine.ShowDialog();
            if (vLine.Result == 1)
            {
                // 新增成功
        ...
        private void lstTrap_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            List<CDevBase> voList = trap.Children();
            foreach (CDevBase basevo in voList)
            {
                CxbDevBranch vo = basevo as CxbDevBranch;
                dgvTrap.Rows.Add(new object[] { vo.StationNames[0], vo.DeviceName, vo.NodeNames[0], vo.NodeNames[1],
                    vo.PosOrNeg, vo.Zr, vo.Z_L, vo.Z_C, trap.DeviceName });
            }
        }

        private void btnDFCAdd_Click(object sender, EventArgs e)
        {
            CzkTxBaseUI vUi = new CzkTxBaseUI();
            vUi.ShowDialog();
        }
    }
}
